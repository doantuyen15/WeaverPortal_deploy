import{aw as k,r as s,j as e,l as y}from"./vendor-B2pREnuB.js";import{B as E,w as L}from"./firebase-BEuHrPfL.js";import{d as j,bi as M}from"./index-DIooF6yI.js";import"./ui-BGi3GnBt.js";import"./charts-B0DTnxZc.js";const h=`<title>Summary and Table, Marked</title>
...
<\/script>
`,b="/*__DATA__*/";function z(t){return JSON.stringify(t).replace(/</g,"<").replace(/\u2028/g,"\u2028").replace(/\u2029/g,"\u2029")}function T(t,n={}){if(!h.includes(b))throw new Error("reviewTemplate.html lost its /*__DATA__*/ placeholder");const r=h.replace(b,z(t));return'<!doctype html><html lang="en"'+(n.theme?' data-theme="'+n.theme+'"':"")+'><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>*{margin:0;padding:0}</style></head><body>'+r+"</body></html>"}function S(t){try{return JSON.parse(t)}catch{throw new Error("Dữ liệu bài chữa không đọc được.")}}const A="writingGradingPrototype",x=t=>`weaver:${M}:writingReview:${t}`;function P(){const{reviewId:t}=k(),[n,r]=s.useState({status:"loading"}),[a,f]=s.useState(""),[p,m]=s.useState(!1),[o,g]=s.useState(!1),d=s.useRef(null);s.useEffect(()=>{let i=!0;return(async()=>{try{const l=await E(L(j,A,t));if(!i)return;if(!l.exists()){r({status:"missing"});return}const u=l.data();r({status:"ready",title:u.title||"Bài chữa",password:String(u.password||""),payloadJson:u.payloadJson||""});try{sessionStorage.getItem(x(t))==="1"&&g(!0)}catch{}}catch(l){i&&r({status:"error",message:l?.message||String(l)})}})(),()=>{i=!1}},[t]),s.useEffect(()=>{n.status==="ready"&&!o&&d.current?.focus()},[n.status,o]);const v=s.useCallback(i=>{if(i.preventDefault(),a.trim()===n.password){g(!0);try{sessionStorage.setItem(x(t),"1")}catch{}}else m(!0),f(""),d.current?.focus()},[a,n.password,t]),w=s.useMemo(()=>{if(!o||n.status!=="ready")return"";try{return T(S(n.payloadJson))}catch(i){return`<!doctype html><meta charset="utf-8"><body style="font:14px system-ui;padding:24px;color:#f28585;background:#14171d">${i.message}</body>`}},[o,n.status,n.payloadJson]);return n.status==="loading"?e.jsx(c,{children:e.jsx("p",{className:"text-sm text-slate-500 dark:text-[#a9b0bd]",children:"Đang tải bài chữa..."})}):n.status==="missing"?e.jsxs(c,{children:[e.jsx("h1",{className:"text-lg font-extrabold text-slate-800 dark:text-[#eef0f4]",children:"Không tìm thấy bài chữa"}),e.jsx("p",{className:"mt-2 text-sm text-slate-500 dark:text-[#a9b0bd]",children:"Đường dẫn có thể đã sai hoặc bài chữa đã được gỡ. Hãy hỏi lại giáo viên gửi link."})]}):n.status==="error"?e.jsxs(c,{children:[e.jsx("h1",{className:"text-lg font-extrabold text-slate-800 dark:text-[#eef0f4]",children:"Không mở được bài chữa"}),e.jsx("p",{className:"mt-2 text-sm text-slate-500 dark:text-[#a9b0bd]",children:n.message})]}):o?e.jsx("iframe",{title:n.title,srcDoc:w,sandbox:"allow-scripts",className:"h-dvh w-full border-0 bg-[#fafbfc] dark:bg-[#14171d]"}):e.jsxs(c,{children:[e.jsx("p",{className:"text-[0.6875rem] font-bold uppercase tracking-[0.14em] text-orange-500 dark:text-[#fb923c]",children:"Weaver · Bài chữa"}),e.jsx("h1",{className:"mt-1.5 text-xl font-extrabold tracking-tight text-slate-800 dark:text-[#eef0f4]",children:n.title}),e.jsx("p",{className:"mt-2 text-sm leading-relaxed text-slate-500 dark:text-[#a9b0bd]",children:"Nhập mật khẩu giáo viên gửi kèm để xem bài chữa."}),e.jsxs("form",{onSubmit:v,className:"mt-5 flex flex-col gap-2.5",children:[e.jsx("label",{htmlFor:"wr-pass",className:"sr-only",children:"Mật khẩu"}),e.jsx("input",{id:"wr-pass",ref:d,type:"password",value:a,autoComplete:"off",onChange:i=>{f(i.target.value),m(!1)},"aria-invalid":p||void 0,"aria-describedby":p?"wr-pass-err":void 0,className:`w-full rounded-xl border bg-white px-3.5 py-2.5 text-sm text-slate-800 outline-none transition
              focus:ring-2 focus:ring-orange-200 dark:bg-[#14171d] dark:text-[#eef0f4] dark:placeholder:text-[#626a79]
              dark:focus:ring-[#3a2410] ${p?"border-red-400 dark:border-[#f28585]":"border-slate-200 focus:border-orange-400 dark:border-[#2b3038] dark:focus:border-[#fb923c]"}`,placeholder:"Mật khẩu"}),p&&e.jsx("p",{id:"wr-pass-err",role:"alert",className:"text-xs font-semibold text-red-500 dark:text-[#f28585]",children:"Mật khẩu chưa đúng. Thử lại nhé."}),e.jsx("button",{type:"submit",disabled:!a.trim(),className:`rounded-xl bg-orange-500 px-4 py-2.5 text-sm font-bold text-white transition
              hover:bg-orange-600 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400
              dark:disabled:bg-[#232830] dark:disabled:text-[#626a79]`,children:"Mở bài chữa"})]})]})}function c(t){const n=y.c(2),{children:r}=t;let a;return n[0]!==r?(a=e.jsx("div",{className:"font-be-vietnam flex min-h-dvh items-center justify-center bg-slate-50 px-5 py-10 dark:bg-[#14171d]",children:e.jsx("div",{className:"w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-[#2b3038] dark:bg-[#1b1f28] dark:shadow-none",children:r})}),n[0]=r,n[1]=a):a=n[1],a}export{P as default};
