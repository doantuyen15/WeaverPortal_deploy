import{ar as k,r,j as n}from"./vendor-BfZpPNdr.js";import{w as y,n as E}from"./firebase-Dgu-Cw_9.js";import{d as L,bg as j}from"./index-yQN62GJX.js";import"./ui-DfpM5-2S.js";import"./charts-eP7_7I19.js";const h=`<title>Summary and Table, Marked</title>
...
<\/script>
`,x="/*__DATA__*/";function M(t){return JSON.stringify(t).replace(/</g,"<").replace(/\u2028/g,"\u2028").replace(/\u2029/g,"\u2029")}function z(t,e={}){if(!h.includes(x))throw new Error("reviewTemplate.html lost its /*__DATA__*/ placeholder");const i=h.replace(x,M(t));return'<!doctype html><html lang="en" data-theme="'+(e.theme||"light")+'"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>*{margin:0;padding:0}</style></head><body>'+i+"</body></html>"}function T(t){try{return JSON.parse(t)}catch{throw new Error("Dữ liệu bài chữa không đọc được.")}}const S="writingGradingPrototype",v=t=>`weaver:${j}:writingReview:${t}`;function I(){const{reviewId:t}=k(),[e,i]=r.useState({status:"loading"}),[l,f]=r.useState(""),[p,m]=r.useState(!1),[s,g]=r.useState(!1),d=r.useRef(null);r.useEffect(()=>{let a=!0;return(async()=>{try{const o=await y(E(L,S,t));if(!a)return;if(!o.exists()){i({status:"missing"});return}const u=o.data();i({status:"ready",title:u.title||"Bài chữa",password:String(u.password||""),payloadJson:u.payloadJson||""});try{sessionStorage.getItem(v(t))==="1"&&g(!0)}catch{}}catch(o){a&&i({status:"error",message:o?.message||String(o)})}})(),()=>{a=!1}},[t]),r.useEffect(()=>{e.status==="ready"&&!s&&d.current?.focus()},[e.status,s]);const b=r.useCallback(a=>{if(a.preventDefault(),l.trim()===e.password){g(!0);try{sessionStorage.setItem(v(t),"1")}catch{}}else m(!0),f(""),d.current?.focus()},[l,e.password,t]),w=r.useMemo(()=>{if(!s||e.status!=="ready")return"";try{return z(T(e.payloadJson))}catch(a){return`<!doctype html><meta charset="utf-8"><body style="font:14px system-ui;padding:24px;color:#8f2b2b">${a.message}</body>`}},[s,e.status,e.payloadJson]);return e.status==="loading"?n.jsx(c,{children:n.jsx("p",{className:"text-sm text-slate-500",children:"Đang tải bài chữa..."})}):e.status==="missing"?n.jsxs(c,{children:[n.jsx("h1",{className:"text-lg font-extrabold text-slate-800",children:"Không tìm thấy bài chữa"}),n.jsx("p",{className:"mt-2 text-sm text-slate-500",children:"Đường dẫn có thể đã sai hoặc bài chữa đã được gỡ. Hãy hỏi lại giáo viên gửi link."})]}):e.status==="error"?n.jsxs(c,{children:[n.jsx("h1",{className:"text-lg font-extrabold text-slate-800",children:"Không mở được bài chữa"}),n.jsx("p",{className:"mt-2 text-sm text-slate-500",children:e.message})]}):s?n.jsx("iframe",{title:e.title,srcDoc:w,sandbox:"allow-scripts",className:"h-dvh w-full border-0 bg-[#fafbfc]"}):n.jsxs(c,{children:[n.jsx("p",{className:"text-[0.6875rem] font-bold uppercase tracking-[0.14em] text-orange-500",children:"Weaver · Bài chữa"}),n.jsx("h1",{className:"mt-1.5 text-xl font-extrabold tracking-tight text-slate-800",children:e.title}),n.jsx("p",{className:"mt-2 text-sm leading-relaxed text-slate-500",children:"Nhập mật khẩu giáo viên gửi kèm để xem bài chữa."}),n.jsxs("form",{onSubmit:b,className:"mt-5 flex flex-col gap-2.5",children:[n.jsx("label",{htmlFor:"wr-pass",className:"sr-only",children:"Mật khẩu"}),n.jsx("input",{id:"wr-pass",ref:d,type:"password",value:l,autoComplete:"off",onChange:a=>{f(a.target.value),m(!1)},"aria-invalid":p||void 0,"aria-describedby":p?"wr-pass-err":void 0,className:`w-full rounded-xl border bg-white px-3.5 py-2.5 text-sm text-slate-800 outline-none transition
              focus:ring-2 focus:ring-orange-200 ${p?"border-red-400":"border-slate-200 focus:border-orange-400"}`,placeholder:"Mật khẩu"}),p&&n.jsx("p",{id:"wr-pass-err",role:"alert",className:"text-xs font-semibold text-red-500",children:"Mật khẩu chưa đúng. Thử lại nhé."}),n.jsx("button",{type:"submit",disabled:!l.trim(),className:`rounded-xl bg-orange-500 px-4 py-2.5 text-sm font-bold text-white transition
              hover:bg-orange-600 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400`,children:"Mở bài chữa"})]})]})}function c({children:t}){return n.jsx("div",{className:"font-be-vietnam flex min-h-dvh items-center justify-center bg-slate-50 px-5 py-10",children:n.jsx("div",{className:"w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-sm",children:t})})}export{I as default};
